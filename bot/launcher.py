...
CONFIG_EXAMPLE_JSON = os.path.abspath(
...
class Configuration:
    ...
    def webhook_count(self):
        ...
        print(
            "You currently have {} webhooks in {} channels with a total amount of {} followed twitter users.".format(
                amount_webhooks, amount_channels, amount_twitter_users
            )
        )
                ...
                try:
                    full_users.extend(
                        self.client.lookup_users(
                            user_ids=twitter_ids[i * 100 : min((i + 1) * 100, user_count)]
                        )
                    )
                except:
    ...
    def add_webhook_via(self, method):
        ...
        if method is "twitter_ids":
            ...
            print(
                "You can follow multiple twitter users by separating the twitter IDs with a comma ,"
            )
    ...
    def modify_webhook(self):
        ...
        if index == 0:
            ...
            while True:
                ...
                choice = user_choice()
                if choice == "1":
                    self.data["Discord"][index]["webhook_urls"] = clean_input(
                        "Give webhook URL: "
                    ).split(",")
                elif choice == "2":
                    while True:
                        ...
                        if choice == "1":
                            ...
                            for twitter_id in twitter_ids:
                                self.data["Discord"][index]["twitter_ids"] += (
                                    [twitter_id]
                                    if twitter_id not in self.data["Discord"][index]["twitter_ids"]
                                    else []
                                )
                        elif choice == "2":
                            twitter_ids = str(input("Give twitter IDs: ").replace(" ", "")).split(
                    ...
                    self.data["Discord"][index]["IncludeRetweet"] = get_bool(
                    ...
                    keyword_sets = [
                        keyword_set.split("+")
                        for keyword_set in keyword_sets.replace(" ", "").split(",")
                    ]
...
if __name__ == "__main__":
    if not PYTHON_OK:
        print(
            "discord-twitter-bot needs Python >=3.5.3, <3.7. You are using: "
            + get_python_version()
        )
        wait()
        exit(1)
    ...
    while True:
        ...
        if discord is None:
            ...
            if discord.version_info.major < 1:
                print(
                    "Warning: You are using the async version of discord.py ({}). This rewrite branch requires you "
                    "to install the rewrite version of discord.py. Running the first option will install the rewrite"
                    "version but your other projects may fail since they require the async version.".format(
                        discord.__version__
                    )
                )
        ...
        if choice == "1":
            ...
            try:
                ...
                print(
                    "Installing discord.py rewrite version failed. Please contact me with reproduction steps."
                )
            wait()
        elif choice == "2":
